...
//                tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_SP, mTextsize);
                ...
                } else if (mTextBold == TEXT_BOLD_WHEN_SELECT && i == 0) {
                    tv_tab_title.getPaint().setFakeBoldText(true);
     ...
     * @param num      num小于等于0显示红点,num大于0显示数字
     */
    public void showMsg(int position, int num, int size) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        ...
        MsgView tipView = (MsgView) tabView.findViewById(R.id.rtv_msg_tip);
        if (tipView != null) {
            UnreadMsgUtils.show(tipView, num, size);
